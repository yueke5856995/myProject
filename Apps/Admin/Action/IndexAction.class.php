...
class IndexAction extends BaseAction {
    ...
    public function toMain(){
    	$this->isLogin();
        $m = D('Index');
        $monthInfo = $m->getMonthInfo();//一月动态
        $this->assign('monthInfo',$monthInfo);
        $sumInfo = $m->getSumInfo();//统计信息
        $this->assign('sumInfo',$sumInfo);
        $todayInfo = $m ->getTodayInfo(); //本日动态
        $this->assign('todayInfo',$todayInfo);
        $yearInfo = $m->getYearInfo();
        $this->assign('yearInfo',$yearInfo); //本年动态
    	$this->display("/main");
    }
    /**
     * 跳去商城配置界面
