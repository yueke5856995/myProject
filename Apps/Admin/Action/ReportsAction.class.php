...
class ReportsAction extends BaseAction{
     ...
     * 去售衣订单统计
     */
    public function toStatCloOrders(){
        $this->isLogin();
        $this->checkPrivelege('dttj_00');
        $this->assign('startDate',date('Y-m-d',strtotime("-31 day")));
        $this->assign('endDate',date('Y-m-d'));

        $this->display("/reports/clo_orders");
    }
    ...
    public function statCloOrders(){
        $this->isLogin();
        $this->checkPrivelege('dttj_00');
        $rs = D('Admin/Reports')->statCloOrders();
        $this->ajaxReturn($rs);
    }
	/**
	 * 去新增会员统计
     ...
     * 去实时统计
     */
    public function toNowReport(){
        $this->isLogin();
        $this->checkPrivelege('dttj_05');
        $this->display("/reports/now_orders");
    }
    /**
     * 实时统计
     */
