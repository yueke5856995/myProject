	 ...
	 */
     ...
     public function queryPenddingByPage(){
        $shopName = WSTAddslashes(I('shopName'));
	 	...
	 	$sql = "select g.*,gc.catName from __PREFIX__goods g 
	 	      ...
	 	      where goodsStatus=0 and goodsFlag=1 and g.isSale=1";
     ...
     public function queryByPage(){
        $shopName = WSTAddslashes(I('shopName'));
     	...
     	$goodsStatus = (int)I('goodsStatus',1);
	 	$sql = "select g.*,gc.catName,p.shopName,u.userName from __PREFIX__goods g 
	 	      left join __PREFIX__goods_cats gc on g.goodsCatId2=gc.catId ,__PREFIX__shops p ,xy_wx_users u
	 	      where  goodsFlag=1 and p.shopId=g.shopId and g.goodsSn = u.userId ";
	 	if($areaId1>0)$sql.=" and p.areaId1=".$areaId1;
	 	if($areaId2>0)$sql.=" and p.areaId2=".$areaId2;
	 	if($goodsCatId1>0)$sql.=" and g.goodsCatId1=".$goodsCatId1;
	 	if($goodsCatId2>0)$sql.=" and g.goodsCatId2=".$goodsCatId2;
	 	...
	 	if($shopName!='')$sql.=" and (p.shopName like '%".$shopName."%' or p.shopSn like '%".$shopName."%')";
	 	if($goodsName!='')$sql.=" and (g.goodsName like '%".$goodsName."%' or g.goodsSn like '%".$goodsName."%' or p.shopName like '%".$goodsName."%')";
	 	if($goodsStatus!='')$sql.=" and g.goodsStatus=".$goodsStatus;
	 	$sql.=" and g.goodsType = 3";
	 	$sql.="  order by goodsId desc";
