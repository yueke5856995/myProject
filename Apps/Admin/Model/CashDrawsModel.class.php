...
class CashDrawsModel extends BaseModel {
    ...
    public function queryByPage($type){
        ...
        $cashSatus = (int)I('cashSatus',-1);
        $sql = "select c.*,u.isVip,u.userName,u.phone from __PREFIX__cash_draws c left join __PREFIX__wx_users u on c.targetType=0 and targetId=u.userId 
                where 1=1";
        if($targetType!=-1){
            $sql.=" and u.isVip=".$targetType;
        }
        if($cashSatus>=0){
            $sql.=" and c.cashSatus=".$cashSatus;
        }
        ...
        if(!empty($rs['root'])){
            foreach ($rs['root'] as $key =>$v){
                $rs['root'][$key]['targetName'] = ($v['isVip']==1?"【会员】":"【非会员】")."【".($v['userName']?$v['userName']:$v['loginName'])."】";
            }
        }
        return $rs;
    }
