    ...
    /**
	 ...
	 public function insert(){
		...
		if($this->checkEmpty($data,true)){
            ...
            $data['priceSection']=I('priceSection');
            $data['priceOut']=I('priceOut');
            $data['catImg']=I('catImg');
            $data['catThumbs']=I('catThumbs');
			$data["catFlag"] = 1;;
			$rs = $this->add($data);
			if($rs){
				$rd['status']= 1;
	  ...
	  */
	  ...
	  public function getCatAndChild(){
	  	  ...
	  	  if($tree)
	  	  	return $tree;
	  	  $data = D('goods_cats')->order('catSort')->select();
