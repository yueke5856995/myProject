...
class UsersModel extends BaseModel {
	...
	public function queryByPage() { 
		...
		if (I ( 'loginName' ) != '')
			$sql .= " and loginName LIKE '%" . WSTAddslashes ( I ( 'loginName' ) ) . "%'";
		if (I ( 'userPhone' ) != '')
			$sql .= " and userPhone LIKE '%" . WSTAddslashes ( I ( 'userPhone' ) ) . "%'";
		if (I ( 'userEmail' ) != '')
			$sql .= " and userEmail LIKE '%" . WSTAddslashes ( I ( 'userEmail' ) ) . "%'";
		if (I ( 'userType', - 1 ) != - 1)
	...
	public function queryAccountByPage() {
		...
		if (I ( 'loginName' ) != '')
			$sql .= " and loginName LIKE '%" . WSTAddslashes ( I ( 'loginName' ) ) . "%'";
		if (I ( 'userStatus', - 1 ) != - 1)
			$sql .= " and userStatus=" . ( int ) I ( 'userStatus', - 1 );
		if (I ( 'userType', - 1 ) != - 1)
			$sql .= " and userType=" . ( int ) I ( 'userType', - 1 );
		$sql .= "  order by userId desc";
		$rs = $this->pageQuery ( $sql );
		// 计算等级
		if (count ( $rs ) > 0) {
	...
	public function editAccount() {
		...
		if (I ( 'loginPwd' ) != '')
			$this->loginPwd = md5 ( I ( 'loginPwd' ) . $loginSecret );
	...
	public function resetPaypwd() {
		...
		if (I ( 'id' ) == '')
			return $rd;
		$loginSecret = $this->where ( "userId=" . ( int ) I ( 'id' ) )->getField ( 'loginSecret' );
		$this->payPwd = md5 ( "888888" . $loginSecret );
		$rs = $this->where ( 'userId=' . ( int ) I ( 'id' ) )->save ();
		if (false !== $rs) {
			$rd ['status'] = 1;
		}
	...
	public function getScoreList() {
		$map = array ();
		$userId = ( int ) I ( "id" );
		$scoreType = ( int ) I ( "scoreType", - 1 );
		$sql = "select us.*,o.orderNo from __PREFIX__user_score us
	 			left join __PREFIX__orders o on us.dataId=o.orderId
	 			where us.userId=$userId ";
		if ($scoreType >= 0) {
			$sql .= " and scoreType=$scoreType ";
		}
	...
	public function getUserScore() {
		$map = array ();
		$userId = ( int ) I ( "id" );
		$sql = "select userScore, loginName,userName from  __PREFIX__users where  userId=$userId ";
	...
	public function editScore() {
		...
		try {
			if ($scoreType == 1) {
				$sql = "update __PREFIX__users set userScore=userScore+$score where  userId=$userId";
			} else {
				$sql = "update __PREFIX__users set userScore=userScore-$score where  userId=$userId";
			}
			$rs = $this->execute ( $sql );
			if (false !== $rs) {
				...
				$data ["dataSrc"] = 5;
				$data ["dataId"] = 0;
				$data ["dataRemarks"] = I ( "dataRemarks" );
				$data ["createTime"] = date ( "Y-m-d H:i:s" );
				$m->add ( $data );
				$rd ['status'] = 1;
				$rd ['msg'] = "操作成功";
	...
	public function getMoneyList() {
		...
		$sql = "select * from __PREFIX__log_moneys
	 			where targetId=$userId and targetType=0 ";
		if ($moneyType >= 0) {
			$sql .= " and moneyType=$moneyType ";
		}
