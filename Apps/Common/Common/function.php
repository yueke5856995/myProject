...
function WSTDataFile($name, $path = '',$data=array()){
	...
	if(is_array($data) && !empty($data)){
		if($data['mallLicense']==''){
			if(stripos($data['mallTitle'],'Powered By HoHu')===false)$data['mallTitle'] = $data['mallTitle']." - Powered By HoHu";
		}
	    $data   =   serialize($data);
...
function WSTOrderNo(){
	$m = M('orderids');
	$orderId = $m->add(array('rnd'=>time()));
	$orderNo = $orderId."".(fmod($orderSrcNo,7));
	return $orderNo;
}
