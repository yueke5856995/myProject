...
class GoodsModel extends BaseModel {
	...
	public function getGoodsList($obj){
		...
		$rs["goodsNav"] = self::getGoodsNav(0,$gcats);
		return $rs;
	}
	 ...
	 * 获取商品相册
	 */
	public function getGoodsImgs($goodsId=0){
		
		$goodsId = ($goodsId>0)?$goodsId:(int)I("goodsId");
	...
	public function queryOnSaleByPage(){
        ...
        $sql = "select g.goodsId,g.goodsSn,g.goodsName,g.goodsImg,g.goodsThums,g.shopPrice,g.goodsStock,g.saleCount,g.isSale,g.isRecomm,g.isHot,g.isBest,g.isNew,g.goodsStatus,g.goodsType,gc.catName from __PREFIX__goods g,__PREFIX__goods_cats gc
				where g.goodsCatId2 = gc.catId and g.goodsFlag=1 
		     and g.shopId=".$shopId."  and g.isSale=1";
		#########################################################
        $mark = I('umark');
        switch ($mark){
            case 'onsale':
                $sql.=" and g.goodsType=3 and g.goodsStatus = 1 " ;
                break;
            case 'unsale':
                $sql.=" and g.goodsType=3 and g.goodsStatus = 0";
                break;
            case 'inbox':
                $sql.=" and g.goodsType=1 and g.goodsStatus = 0";
                break;
            case 'onroad':
                $sql.=" and g.goodsType=2 ";
                break;
            case 'outsale':
                $sql.=" and g.goodsType=3 and g.goodsStatus >1";
                break;
		...
		$sql.=" order by g.goodsStatus asc,g.goodsId desc";
		$pager = $this->pageQuery($sql);
		//var_dump($this->getDbError());
		$root = $pager['root'];
		for($i=0,$k=count($root);$i<$k;$i++){
			$root[$i]["kcode"] = md5(base64_encode("wstmall".date("Y-m-d")));
		}
		$pager['root'] = $root ;
		return $pager;
	}
	...
	public function insert(){
		...
		if ($m->create()){
			...
			try {
				...
				$m->goodsType = 3;
				$m->goodsUnit = I('goodsUnit');
				$m->goodsSpec = I('goodsSpec');
                $m->saleTime = I('saleTime');
				$goodsId = $m->add();
				if(false !== $goodsId){
					if($shopStatus[0]['shopStatus']==1){
					    $rd['status']= 1;
	 ...
	 */
	 public function get(){
		...
		//获取卖家信息 其中设定 goodsSn 为卖家的userId
         $m=M('wx_users');
         $user=$m->alias('u')->field('u.userId,u.userName,u.phone,addr.address')->join("__USER_ADDRESS__ addr on u.userId = $goods[goodsSn]")
                    ->where('addr.isDefault = 1')->find();
         $goods['user'] = $user;

        //获取买家信息 其中设定 orderId 为买家订单Id
         if($goods['goodsStatus']>=4){
            $buyer=M('orders_clothes')->field('userId,getName,getAddress,getPhone,orderRemarks')->find($goods['orderId']);
             $goods['buyer'] = $buyer;
         }
	 ...
	 public function del(){
		...
		M()->startTrans();
		try {
		 	$rs = $m->where("shopId=".$shopId." and goodsId=".$goodsId)->save($data);
		    if(false !== $rs){
				$rd['status']= 1;
				M('cart')->where("goodsId =".$goodsId)->delete();
			}
			M()->commit();
		} catch (\Exception $e) {
			M()->rollback();
	 ...
	 public function batchDel(){
		...
		try {
		    ...
		    if(false !== $rs){
				$rd['status']= 1;
				M('cart')->where(" goodsId in(".$ids.")")->delete();
			}
			M()->commit();
		} catch (\Exception $e) {
			M()->rollback();
	 ...
	 public function sale(){
        ...
        $rs = $m->where("shopId=".$shopId." and goodsId in(".$ids.")")->save($data);
		    if(false !== $rs){
                ...
                $good=M('goods');
                $goodsList=$good->field('goodsSn,goodsName')->where("goodsId in($ids)")->select();
                ...
                switch ($type) {
                    case 1://审核
                        ...
                        break;
                    case 2://下架
                        $text = "您上架的商品被管理员下架，请在衣物存储中查看";
                        break;
                    case 3:
                        $text = '您购买的商品已发货，请在衣物交易中查看';
                        break;
                    case 4:
                        $text = '您的衣物正在被送回，请在衣物存储中查看';
                        break;
                    case 5:
                        $text = '您的衣物正在被送回，请在衣物存储中查看';
                        break;
                    case 6://拒绝
                        $text = "您上架的商品未通过审核，请在衣物存储中查看";
                        break;
                }

                foreach ($goodsIds as $key=>$v){
                    $yj_data = array(
                        'msgType' => 0,//0系统 //1 洗衣 //2 储存 //3 物流 //4交易
                        'sendUserId' => session('WST_STAFF.staffId'),
                        'receiveUserId' => $v['goodsSn'],
                        'msgContent' => $text,
	 ...
	 * 获取商品类别导航
	 */
	public function getGoodsNav($goodsId=0,$obj=array()){
		if($goodsId>0){
	...
	function getViewGoods(){
		...
		if(!empty($viewGoods)){
			$ids = array();
			for($i=0,$k=count($viewGoods);$i<$k;$i++){
				$ids[] = (int)$viewGoods[$i];
			}
			$goodIds = implode(",",$ids);
		}
