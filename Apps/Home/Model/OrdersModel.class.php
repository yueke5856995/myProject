...
class OrdersModel extends BaseModel {
	...
	public function orderRead(){
		...
		$sql = "SELECT orderId,orderStatus,payType,isPay FROM __PREFIX__orders WHERE orderFlag=1 and orderId = $orderId and shopId=".$shopId;
		$rs = $this->queryRow($sql);
		$list = array(-1,-3,-6,-9);
		$status = array("-1"=>-8,"-3"=>-4,"-6"=>-7,"-9"=>-10);
		if(($rs["payType"]==0 || $rs["isPay"]==0) && in_array($rs["orderStatus"], $list)){
			$where = array();
			$where["orderFlag"] = 1;
	...
	public function queryShopOrders($obj){
		...
		$rsdata = array();
		if($orderStatus>=4){
		$sql = "SELECT isPay,payType,orderNo,orderId,userId,getName userName,getAddress userAddress,totalMoney,realTotalMoney,scoreMoney,orderStatus,createTime,refundRemark,orderFrom,orderFromId FROM __PREFIX__orders WHERE  orderFlag=1 and shopId = $shopId ";
		}else{
            $sql = "SELECT isPay,payType,orderNo,orderId,userId,userName,userAddress,totalMoney,realTotalMoney,scoreMoney,orderStatus,createTime,refundRemark,orderFrom,orderFromId FROM __PREFIX__orders WHERE  orderFlag=1 and shopId = $shopId ";
        }
		//自写代码开始
		if($orderStatus==1   ){//下单成功
			$sql.=" AND (orderStatus=1)";
		}else if($orderStatus==2){//上门取衣
			$sql.=" AND (orderStatus=2)";
		}else if($orderStatus==3){//取衣成功
			$sql.=" AND (orderStatus=3)";
		}else if($orderStatus==4){//正在清洗
			$sql.=" AND (orderStatus=4)";
		}else if($orderStatus==5){//清洗完成
			$sql.=" AND orderStatus=5";
		}else if($orderStatus==6){//物流运送
            $sql.=" AND orderStatus=6";
        }else if($orderStatus==7){//上门送衣
            $sql.=" AND orderStatus=7";
        }else if($orderStatus==8){//送衣完成
            $sql.=" AND orderStatus=8";
        }else if($orderStatus==9){//订单完成
            $sql.=" AND orderStatus=9";
        }else{//取消/退款
