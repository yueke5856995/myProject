...
class ReportsModel extends BaseModel {
	...
	public function getTopSaleGoods(){
		...
		$rs = M('order_goods og')->field('og.goodsId,g.goodsName,goodsSn,sum(og.goodsNums) goodsNums,og.goodsThums')
				...
				->limit(20)->select();
		//return M('order_goods')->getLastSql();
		return array("data"=>$rs?$rs:array());
	}
	...
	public function getStatSales(){
		...
		$shopId = (int)session('WST_USER.shopId');
		$rs = M('orders')->field('left(createTime,10) createTime,sum(totalMoney) totalMoney,count(orderId) orderNum,sum(useCoupons) conpons')
		...
		if(count($rs)>0){
			...
			foreach($rs as $key => $v){
			    //自写
                $conpous=$v['conpons'];
                $choucheng=$v['totalMoney']*0.5;
                $jiesuan=$v['totalMoney']*0.5;
                //结束
				$days[] = $v['createTime'];
				$rdata['dayVals'][] = $jiesuan;//$v['totalMoney'];
				$rdata['list'][] = array('day'=>$v['createTime'],'val'=>$v['totalMoney'],'conpous'=>$conpous,'choucheng'=>$choucheng,'jiesuan'=>$jiesuan,'num'=>$v['orderNum']);
			}
			$rdata['days'] = $days;
		}
		return $rdata;
