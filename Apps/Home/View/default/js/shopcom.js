...
function toEditGoods(id, menuId) {
    ...
    location.href = Think.U('Home/Goods/toEdit', {"umark": menuId, "id": id, "p": pageNo});
}
...
function delGoods(id, islast, method) {
        ...
        $.post(Think.U('Home/Goods/del'), {id: id}, function (data, textStatus) {
            ...
            if (json.status == '1') {
                WST.msg('操作成功！', {icon: 1}, function () {
                    ...
                    if (islast == 1 && pageNo > 1) {
                        $("#pageNo").val(pageNo - 1);
                        location.href = Think.U('Home/Goods/' + method, {"p": (pageNo - 1)})
                    } else {
                        location.reload();
                    }
...
function sale(v) {
    ...
    layer.confirm(text, {icon: 3, title: '系统提示'}, function (tips) {
        var loading = layer.load('正在处理，请稍后...', 3);
        layer.close(tips);
        var params = {};
        params.ids = ids;
...
function editGoods(menuId) {
    ...
    if (params.shopPrice <= 0) {
        WST.msg('店铺价格必须大于0!', {icon: 5});
        return;
    }
    if(params.marketPrice*1< params.shopPrice*1){
        WST.msg('现价不得大于原价!', {icon: 5});
        return;
    }
    if (params.isDistribut == 1 && params.commission <= 0) {
        WST.msg('分销佣金不能为0!', {icon: 5});
        return;
    }
        ...
        if (json.status == '1') {
            WST.msg('操作成功!', {icon: 1}, function () {
                /*if ((menuId == 'toEditGoods')) {
                    location.href= Think.U('Admin/Goods/index','umark=toEditGoods');
                } else {
                    location.href=Think.U('Admin/Goods/index',{"p":pageNo});
                }*/
            });
        } else if (json.status == '-2') {
            if (params.isSale == 1) {
                WST.msg('您的店铺已被封，如有疑问请与商城管理员联系!', {icon: 5, time: 4000}, function () {
                    if ((menuId == 'toEditGoods')) {
                        ...
                        //location.href=Think.U('Home/Goods/'+menuId,{"p":pageNo});
                    }
                });
            } else {
                WST.msg('操作成功!', {icon: 1}, function () {
...
function queryOrderPager(statusMark, pcurr) {
    ...
    param.statusMark = statusMark + 1;
    param.pcurr = pcurr;
    var ll = layer.load('数据加载中，请稍候...');
    $.post(Think.U('Home/Orders/queryShopOrders'), param, function (data, textStatus) {
        var json = WST.toJson(data);
        $("#wst-tab" + statusMark + " tbody").remove();
        var tmpMsg = '';
        if (json.root.length > 0) {
            for (var i = 0; i < json.root.length; i++) {
                var order = json.root[i];
                var html = new Array();
                html.push('<tbody class="j-order-row">');
                html.push('<tr class="empty-row"><td colspan="5">&nbsp;</td></tr>');
                html.push('<tr class="order-head">');
                html.push('<td colspan="5" align="right"><div class="time">' + order.createTime + '</div><div class="orderno">订单号：' + order.orderId);

                html.push('</div>');
                /*var payType = WST.PayType(order.payType);*/
                html.push("&nbsp;<a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=showOrder('" + order.orderId + "')>查看订单</a><br/>");
                html.push('</td>');
                html.push('</tr>');
                for (var j = 0; j < order.goodslist.length; j++) {
                    var goods = order.goodslist[j];
                    html.push('<tr class="goods-box">');
                    if (j == 0) {
                        html.push("<td width='20' rowspan='" + order.goodslist.length + "'><input type='checkbox' class='chk_" + ((statusMark == 5) ? 5 : order.orderStatus) + "' value='" + order.orderId + "'/></td>");
                    }
                    var goodsUrl = Think.U('Home/Goods/getGoodsDetails', {'goodsId': goods.goodsId});
                    if (order.orderFrom == 2) {
                        goodsUrl = Think.U('Home/Groups/getGoodsDetails', {'id': order.orderFromId});
                    } else if (order.orderFrom == 3) {
                        goodsUrl = Think.U('Home/Panics/getGoodsDetails', {'id': order.orderFromId});
                    }
                    html.push('<td>');
                    html.push('<div class="goods-img"><a target="_blank" href="' + goodsUrl + '"><img data-original="' + WST.ROOT + '/' + goods.goodsThums + '" class="olazyImg" title="' + goods.goodsName + '"/></a></div>');
                    html.push('<div class="goods-name">');
                    html.push('<div><a target="_blank" style="color:' + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? 'red' : '') + ';" href="' + goodsUrl + '">' + goods.goodsName + '</a></div>');
                    html.push('<div style="color:' + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? 'red' : '') + ';">' + goods.goodsAttrName + '</div>');
                    html.push('</div>');
                    html.push('<div class="goods-extra" style="color:' + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? 'red' : '') + ';">' + goods.goodsPrice + ' x ' + goods.goodsNums + '</div>');
                    html.push('</td>');
                    if (j == 0) {
                        html.push('<td rowspan="' + order.goodslist.length + '">');
                        if (order.orderStatus < 0) {
                            html.push('<div style="color:' + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? 'red' : '') + ';">' + (order.refundRemark != "" ? WST.cutStr(order.refundRemark, 40) : "--") + '</div>');
                        } else {
                            html.push('<div style="color:' + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? 'red' : '') + ';">' + order.userName + "<br/>" + order.userAddress + '</div>');
                            html.push('<div style="color:' + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? 'red' : '') + ';">' + '</div>');
                        }

                        html.push('</td>');
                        html.push('<td width="120" rowspan="' + order.goodslist.length + '">');
                        html.push('<div style="color:' + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? 'red' : '') + ';">');
                        html.push('<div >' + order.totalMoney + '</div>');
                        /*if(order.scoreMoney>0){
                         html.push('<div >积分抵扣：￥'+order.scoreMoney+'</div>');
                         }*/

                        html.push('</div>');
                        html.push('</td>');
                        html.push('<td width="80" rowspan="' + order.goodslist.length + '">');
                        if (order.orderStatus == 1) {
                            html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=changeOrderStatus(" + order.orderId + ",2)>【上门取衣】</a></div>");
                        } else if (order.orderStatus == 2) {
                            html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=changeOrderStatus(" + order.orderId + ",3)>【取衣成功】</a></div>");
                        } else if (order.orderStatus == 3) {
                            html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=changeOrderStatus(" + order.orderId + ",4)>【正在清洗】</a></div>");
                        } else if (order.orderStatus == 4) {
                            html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=changeOrderStatus(" + order.orderId + ",5)>【清洗完成】</a></div>");
                        } else if (order.orderStatus == 5) {
                            html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=changeOrderStatus(" + order.orderId + ",7)>【上门送衣】</a></div>");
                        } else if (order.orderStatus == 7) {
                            html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=changeOrderStatus(" + order.orderId + ",8)>【送衣完成】</a></div>");
                        } else if (order.orderStatus == 8) {
                            html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=changeOrderStatus(" + order.orderId + ",9)>【订单完成】</a></div>");
                        }
                        /*if(order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9){
                         if(order.payType>=1 && order.isPay==1){
                         html.push('<div><a href="javascript:;" style="color:'+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?'red':'')+';" onclick=refund("'+order.orderId+'")>【操作退款】</a></div>');
                         }else{
                         html.push('<div><a href="javascript:;" style="color:'+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?'red':'')+';" onclick=orderRead("'+order.orderId+'")>【标记已读】</a></div>');
                         }
                         }*/
                        html.push("<div><a href='javascript:;' style='color:" + ((order.orderStatus == -1 || order.orderStatus == -3 || order.orderStatus == -6 || order.orderStatus == -9) ? "red" : "") + "' onclick=orderPrint('" + order.orderId + "')>【打印订单】</a></div>");
                        html.push('</td>');
                    }
                ...
                $(html.join("")).insertBefore("#wst-tab" + statusMark + " tfoot");
                $('.olazyImg').lazyload({
                    effect: "fadeIn",
                    failurelimit: 10,
                    skip_invisible: false,
                    threshold: 200,
                    placeholder: WST.ROOT + '/' + WST.GOODS_LOGO
                });
            }
            if (json.totalPage > 1) {
                laypage({
...
function orderRead(id) {
    ...
    $.post(WST.U('Home/Orders/orderRead'), {"orderId": id}, function (data) {
        layer.close(ll);
        var json = WST.toJson(data);
        if (json.status == 1) {
...
function refund(id) {
    ...
    $.post(WST.U('Home/Orders/toShopRefund'), {"orderId": id}, function (data) {
        layer.close(ll);
        var w = WST.open({
            type: 1,
            title: "退款操作",
            ...
            yes: function (index, layero) {
                ...
                params.refundStatus = $("input[name='refundStatus']:checked").val();
                ...
                $.post(WST.U('Home/Orders/shopOrderRefund'), params, function (data) {
                    layer.close(ll);
                    var json = WST.toJson(data);
                    if (json.status == 1) {
                        WST.msg(json.msg, {icon: 1});
...
function editShop() {
    ...
    params.fenhong = $('#fenhong').val();
    params.t_fenhong = $('#t_fenhong').val();
    console.log(params);
    var relateArea = [0];
...
function logout() {
    jQuery.post(Think.U('Home/Shops/logout'), {}, function (rsp) {
        ...
        //location.href = Think.U('Home/shop/login');
    });
}
function checkLogin() {
...
function getCurrPackage(obj) {
    ...
    if ($("#package_" + packageId).hasClass("currRow")) {
        ...
        jQuery.post(Think.U('Home/Goods/getPackageGoods'), {"goodsIds": goodsIds}, function (data) {
            ...
            for (var i = 0; i < json.length; i++) {
                var obj = json[i];
                html.push("<option value='" + obj.goodsId + "' price='" + obj.shopPrice + "' txt='" + obj.goodsName + "' diffPrice='" + prices[i] + "'>" + obj.goodsName + "【差价：" + prices[i] + "】</option>");
            }
...
function loadMoneys(p) {
    ...
    params.p = p;
    $.post(Think.U('Home/LogMoneys/loadMoneys'), params, function (data, textStatus) {
        layer.close(tips);
        var json = WST.toJson(data);
        ...
        if (json.status == 1) {
            ...
            var w = layer.open({
                ...
                yes: function (index, layero) {
                    ...
                    $.post(Think.U('Home/CashDraws/drawsCash'), params, function (data) {
                        layer.close(ll);
                        var json = WST.toJson(data);
                        if (json.status > 0) {
                            $('#drawForm')[0].reset();
...
function getAreaList(objId, parentId, t, id) {
    ...
    $('#' + objId).empty();
    if (parentId == 0) {
        $('#' + objId).empty().html('<option value="">请选择</option>');
        return false;
    }
    var html = [];
...
function getShopMoneys() {
    $.post(Think.U('Home/LogMoneys/getShopMoneys'), {}, function (data, textStatus) {
        var json = WST.toJson(data);
        if (json.status == '1') {
            $('#shopMoney').html(json.shopMoney);
...
function batchPrint(status) {
    ...
    location.href = Think.U('Home/Orders/printOrders', {"orderIds": ids});
}

function orderPrint(orderId) {
    location.href = Think.U('Home/Orders/printOrders', {"orderIds": orderId});
}
...
function changeOrderStatus(orderId, status) {
    ...
    layer.confirm('您确定进行该操作？', {icon: 3, title: '系统提示'}, function (tips) {
        var ll = layer.load('数据处理中，请稍候...');
        $.post(Think.U('Home/Orders/changeOrderStatus'), {orderId: orderId, status: status}, function (data) {
            layer.close(ll);
            layer.close(tips);
            var json = WST.toJson(data);
            if (json.status > 0) {
