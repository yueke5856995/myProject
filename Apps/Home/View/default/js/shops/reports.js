...
function getTopSaleGoods(){
	...
	$.post(Think.U('Home/Reports/getTopSaleGoods'),params,function(data,textStatus){
		...
		$('.glazyImg').lazyload({ effect: "fadeIn",failurelimit : 10,skip_invisible : false,threshold: 200,placeholder:WST.DEFAULT_IMG});
		...
		var option = {
			...
			},
			{
				...
				itemStyle :　{
			 	...
			 	data:[
			 		{value:(json.map['-3']?json.map['-3']:0), name:'取消订单'},
			   		{value:(json.map['-1']?json.map['-1']:0), name:'退款订单'},
			    	{value:(json.map['1']?json.map['1']:0), name:'正常订单'},
				]
			}]
		};            
		myChart.setOption(option);	 
