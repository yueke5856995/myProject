...
class OrdersAction extends BaseAction {
	...
	public function editOrderDetails(){
		...
		$data["orderInfo"] = $rs;
		$this->assign("orderInfo",$rs);
		$this->display("users/orders/edit_details");
	}

	public function editOrder(){
		$morders = D('Home/Orders');
		$obj["orderId"] = (int)I("orderId");	
		$obj["payType"] = (int)I("payType");
		$obj["isSelf"] = (int)I("isself");
		$obj["orderRemarks"] = I("remarks");
		$rs = $morders->editOrder($obj);
		$this->ajaxReturn($rs);
	}
	...
	*/
    ...
    public function batchShopOrderAccept(){
    	$this->isShopLogin();
    	$morders = D('Home/Orders');
		$rs = $morders->batchShopOrderAccept();
		$this->ajaxReturn($rs);
	}
	/**
	 * 商家生产订单
	...
	public function batchShopOrderProduce(){
    	$this->isShopLogin();
    	$morders = D('Home/Orders');
		$rs = $morders->batchShopOrderProduce();
		$this->ajaxReturn($rs);
	} 
	/**
	 * 商家发货配送订单
    ...
    public function batchShopOrderDelivery(){
    	$this->isShopLogin();
    	$morders = D('Home/Orders');
		$rs = $morders->batchShopOrderDelivery();
		$this->ajaxReturn($rs);
	}
