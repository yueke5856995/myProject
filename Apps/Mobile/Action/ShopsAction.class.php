...
/* 添加店铺滚动广告 */
    public function editShopCfg1(){
        $shopId=session('WST_USER.shopId');
        $mc = M('shop_configs');

        $shopcg = $mc->where('shopId='.$shopId)->find();

        if(!empty($shopcg['shopAds'])){
            $shopAds['shopAds']=$shopcg['shopAds'].'#@#'.I('shopAds');
        }else{
            $shopAds['shopAds']=I('shopAds');
        }

        if(!empty($shopcg['shopAdsUrl'])){
            $shopAds['shopAdsUrl']=$shopcg['shopAdsUrl'].'#@#'.I('shopAdsUrl');
        }else{
            $shopAds['shopAdsUrl']=I('shopAdsUrl');
        }

        $rs = $mc->where("shopId=".$shopId)->save($shopAds);
        $this->ajaxReturn($rs);
    }
...
/* 删除广告 */
    public function delAd(){
        ...
        $ads=str_replace($img.'#@#','',$shopcg['shopAds']);

        if($ads==$shopcg['shopAds']){
            $ads=str_replace('#@#'.$img,'',$shopcg['shopAds']);
        }
        $rs = $mc->where("shopId=".$shopId)->save(array("shopAds"=>$ads));
    ...
    */
    ...
    public function Pass(){
        ...
        if($rs['loginPwd']==md5($pass.$rs['loginSecret'])){
            $this->ajaxReturn("1");
        }else{
            $this->ajaxReturn("0");
        }
