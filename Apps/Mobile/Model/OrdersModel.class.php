...
class OrdersModel extends BaseModel {
     ...
     * 修改订单详情
     */
	public function editOrder($obj){
		$orderId = (int)$obj["orderId"]  ;	
		$payType = (int)$obj["payType"]  ;
		$isSelf = (int)$obj["isSelf"]  ;
		$orderRemarks = $obj["orderRemarks"]  ;
		if($obj['payType']=="0"){
			$orderStatus = 0;
		}else{
			$orderStatus = -2;
		...
		$sql = "UPDATE  __PREFIX__orders SET payType=$payType,isSelf=$isSelf,orderRemarks='".$orderRemarks."',orderStatus=$orderStatus WHERE orderId = $orderId ";	
		$rs = $this->execute($sql);
		return $rs;
	}
	...
	public function queryShopOrders($obj){
		...
		$rsdata = array();
		$sql = "SELECT isSelf,isPay,payType,orderNo,orderId,userId,userName,userAddress,totalMoney,realTotalMoney,scoreMoney,orderStatus,createTime,refundRemark,orderFrom,orderFromId FROM __PREFIX__orders WHERE  orderFlag=1 and shopId = $shopId ";
