...
function editOrder(id){
	layer.open({
	    ...
	    shade: [0.6, '#000'],
	    border: [0],
	    content: [Think.U('Home/Orders/editOrderDetails','orderId='+id)],
	    area: ['1020px', ($(window).height() - 50) +'px']
	});
}
...
function queryOrderPager(statusMark,pcurr){
	...
	$.post(Think.U('Home/Orders/queryShopOrders'),param,function(data,textStatus){
			...
			if(json.root.length>0){
				for(var i=0;i<json.root.length;i++){
					...
					html.push('<tbody class="j-order-row">');
						...
						html.push('<tr class="order-head">');
							html.push('<td colspan="5" align="right"><div class="time">'+order.createTime+'</div><div class="orderno">订单号：'+order.orderNo);
							if(order.orderFrom==2){
								html.push('&nbsp;<img src="'+WST.ROOT +'/Apps/Home/View/'+WST.WST_STYLE+'/images/tuan.png" height="30" />');
							}else if(order.orderFrom==3){
								html.push('&nbsp;<img src="'+WST.ROOT +'/Apps/Home/View/'+WST.WST_STYLE+'/images/qian.png" height="30" />');
							}
							html.push('</div>');
							var payType = WST.PayType(order.payType);
							html.push(payType+"&nbsp;|&nbsp;<a href='javascript:;' style='color:"+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?"red":"")+"' onclick=showOrder('"+order.orderId+"')>查看订单</a><br/>");
							html.push('</td>');
						html.push('</tr>');
								...
								if(j==0){
									html.push('<td rowspan="'+order.goodslist.length+'">');
										if(order.orderStatus<0){
											...
											html.push('<div style="color:'+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?'red':'')+';">'+(order.isSelf==1?"自提":"送货上门")+'</div>');
										}
									...
									html.push('<td width="120" rowspan="'+order.goodslist.length+'">');  
										...
										html.push('<div >'+order.realTotalMoney+'</div>');
									...
									html.push('<td width="80" rowspan="'+order.goodslist.length+'">');
										if(order.orderStatus==0){
											html.push("<div><a href='javascript:;' style='color:"+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?"red":"")+"' onclick=editOrder('"+order.orderId+"')>【修改订单】</a></div>");
											html.push("<div><a href='javascript:;' style='color:"+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?"red":"")+"' onclick=shopOrderAccept('"+order.orderId+"')>【受理订单】</a></div>");
										}else if(order.orderStatus==1){
											html.push("<div><a href='javascript:;' style='color:"+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?"red":"")+"' onclick=shopOrderProduce('"+order.orderId+"')>【打包订单】</a></div>");
										}else if(order.orderStatus==2){
											html.push("<div><a href='javascript:;' style='color:"+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?"red":"")+"' onclick=shopOrderDelivery('"+order.orderId+"')>【发货配送】</a></div>");
										}
				...
				$('.olazyImg').lazyload({ effect: "fadeIn",failurelimit : 10,skip_invisible : false,threshold: 200,placeholder:WST.ROOT+'/'+WST.GOODS_LOGO});
			}
			if(json.totalPage>1){
	           laypage({
