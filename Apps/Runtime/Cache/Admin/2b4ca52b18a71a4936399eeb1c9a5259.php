        ...
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            ...
            <?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><!-- 一级分类start -->
             ...
             <!-- 二级分类start -->
             <?php if($vo['childNum'] > 0): if(is_array($vo['child'])): $i2 = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i2 % 2 );++$i2;?><tr id='tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>' class="tr_0_<?php echo ($i); ?>" isLoad='1' hidden>
	               <td>
	               ...
	               <td>
	               <?php if(in_array('spfl_01',$WST_STAFF['grant'])){ ?>
	               <a class="btn btn-default glyphicon glyphicon-plus" href="<?php echo U('Admin/GoodsCats/toEdit',array('parentId'=>$vo2['catId']));?>">新增品牌</a>&nbsp;
	               <?php } ?>
	               <?php if(in_array('spfl_02',$WST_STAFF['grant'])){ ?>
	             ...
	             <!-- 三级分类start -->
                 <?php if($vo2['childNum'] > 0): if(is_array($vo2['child'])): $i3 = 0; $__LIST__ = $vo2['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo3): $mod = ($i3 % 2 );++$i3;?><tr id='tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>_<?php echo ($i3); ?>' class="tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>" isLoad='1' hidden>
		               <td>
