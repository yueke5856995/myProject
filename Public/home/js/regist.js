...
function onfocusName(obj){
	...
	jQuery("#loginNameTip").html("<span>请输入邮箱/用户名(5-20位)/手机号</span>");
}
...
function changeName(){
	...
	if(params.loginName!="" && params.loginName!="邮箱/用户名(5-20位)/手机号"){
	...
	getVerify();
	 $.formValidator.initConfig({
		   theme:'Default',mode:'AutoTip',formID:"register",debug:true,submitOnce:true,onSuccess:function(){
			...
			if(uname=="邮箱/用户名(5-20位)/手机号"){
				jQuery("#loginNameTip").removeClass();			
				jQuery("#loginNameTip").addClass("onError");
				jQuery("#loginNameTip").html("请输入邮箱/用户名(5-20位)/手机号");
			}
		}});
